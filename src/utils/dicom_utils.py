...
def get_labels(contours, shape, dicom_info, rois=None):
    ...
    if not contours == []:
        ...
        if rois is None:
            label_map = np.zeros([shape[0], shape[1], shape[2], len(contours)], dtype="bool")
            num = 1
            ct = 0
            for con in contours:
            ...
            if type(rois) == list:
                if len(rois) <= 1:
                    label_map = np.zeros([shape[0], shape[1], shape[2]], dtype="bool")
                else:
                    label_map = np.zeros([shape[0], shape[1], shape[2], len(rois)], dtype="bool")
            elif type(rois) == str:
                label_map = np.zeros([shape[0], shape[1], shape[2]], dtype="bool")
...
def postprocess(pred, num_volume):
    ...
    if num_object > num_volume:
        ...
        pred = morphology.remove_small_objects(pred, int(unique_volume[-num_volume-1] - 1))
